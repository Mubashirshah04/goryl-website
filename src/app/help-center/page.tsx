...
export default function HelpCenter() {
  ...
  return (
    <>
      ...
      />
      <Script 
        src="https://files.bpcontent.cloud/2025/10/23/08/20251023080752-F2VBPIXR.js"
        strategy="afterInteractive"
        defer
      />
            ...
            <div className="max-w-2xl mx-auto">
              <div className="relative">
                <input
                  ...
                  className="w-full px-6 py-4 pl-12 pr-20 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {popularTopics.map((topic, index) => (
            <motion.div key={topic.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 + index * 0.1 }} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow cursor-pointer group">
      ...
      <div className="bg-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: <MessageCircle className="w-8 h-8" />, title: 'Live Chat', description: 'Chat with our support team in real-time', action: 'Available 24/7', color: 'bg-blue-500 hover:bg-blue-600' },
              { icon: <Mail className="w-8 h-8" />, title: 'Email Support', description: 'Send us a detailed message', action: 'Send Email', color: 'bg-green-500 hover:bg-green-600', onClick: () => window.open('mailto:support@goryl.com') },
              { icon: <Phone className="w-8 h-8" />, title: 'Phone Support', description: 'Speak directly with our team', action: 'Call Now', color: 'bg-purple-500 hover:bg-purple-600', onClick: () => window.open('tel:+1800467955') }
            ].map((method, index) => (
              <motion.div key={method.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 + index * 0.1 }} className="text-center">
                ...
                <button 
                  ...
                  className={`${method.color} text-white px-6 py-3 rounded-lg font-medium transition-colors hover:scale-105 transform`}
                >
                  {method.action}
                </button>
              </motion.div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7 + index * 0.1 }} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              <button onClick={() => setExpandedFaq(expandedFaq === index ? null : index)} className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                ...
                {expandedFaq === index ? <ChevronUp className="w-5 h-5 text-gray-500 dark:text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-500 dark:text-gray-400" />}
              </button>
              {expandedFaq === index && (
