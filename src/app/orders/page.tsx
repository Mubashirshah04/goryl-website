...
interface Order {
  ...
  buyerUid: string;
  userId: string;
  sellerRef?: { id: string; path: string };
  items: any[];
  shipping: any;
  total: number;
  totalAmount?: number;
...
export default function OrdersPage() {
  ...
  const showOrderTrackingInfo = (order: Order) => {
...
🏪 Goryl Marketplace - Track Your Order
  ...
  const handleRateOrder = async (orderId: string) => {
    ...
    const rating = await new Promise<string | null>((resolve) => {
      const modal = document.createElement('div');
      modal.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
      modal.innerHTML = `
        <div class="bg-white rounded-lg p-6 max-w-md mx-4">
      ...
      modal.querySelector('#confirm')?.addEventListener('click', () => {
        const ratingValue = (modal.querySelector('#rating') as HTMLInputElement)?.value;
        document.body.removeChild(modal);
        resolve(ratingValue || null);
      });
    });
    
    const review = await new Promise<string | null>((resolve) => {
      const modal = document.createElement('div');
      modal.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
      modal.innerHTML = `
        <div class="bg-white rounded-lg p-6 max-w-md mx-4">
      ...
      modal.querySelector('#confirm')?.addEventListener('click', () => {
        const reviewValue = (modal.querySelector('#review') as HTMLTextAreaElement)?.value;
        document.body.removeChild(modal);
...
🏪 Goryl Customer Care
  ...
  const handleReorder = (orderId: string) => {
    const order = orders.find(o => o.id === orderId);
    if (order) {
      toast.info(`🛒 Adding ${order.items?.length || 0} items to your cart...`);
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-300">Track and manage your purchases</p>
        </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-300">
                        {order.paymentMethod === 'card' ? 'Card' : 
                         ...
                         order.payment?.method === 'online' ? 'Card' : 'COD'}
                      </p>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button 
                          ...
                          className="flex items-center bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-medium text-sm transition-colors"
                        >
                        ...
                        {(order.status === 'confirmed' || order.status === 'shipped' || order.status === 'delivered') && (
                          <button 
                            onClick={() => showOrderTrackingInfo(order)}
                            className="flex items-center bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 font-medium text-sm transition-colors"
                          >
                        ...
                        {order.status === 'delivered' && (
                          <button 
                            ...
                            className="flex items-center text-orange-600 hover:text-orange-700 font-medium text-sm border border-orange-600 px-4 py-2 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors"
                          >
                        ...
                        {order.status === 'delivered' && (
                          <button 
                            ...
                            className="flex items-center text-yellow-600 hover:text-yellow-700 font-medium text-sm border border-yellow-600 px-4 py-2 rounded-lg hover:bg-yellow-50 dark:hover:bg-yellow-900/20 transition-colors"
                          >
                      ...
                      <div className="flex gap-2">
                        <button 
                          ...
                          className="flex items-center border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 font-medium text-sm transition-colors"
                        >
                  ...
                  {order.timeline && order.timeline.length > 0 && (
                    <div className="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4">
                      ...
                      <div className="space-y-2">
                        {order.timeline.map((event, eventIndex) => (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {selectedOrder.shippingAddress?.company && (
                    <p className="text-gray-600 dark:text-gray-400">{selectedOrder.shippingAddress.company}</p>
                  )}
                  <p className="text-gray-600 dark:text-gray-400">{selectedOrder.shippingAddress?.addressLine1}</p>
                  {selectedOrder.shippingAddress?.addressLine2 && (
                    <p className="text-gray-600 dark:text-gray-400">{selectedOrder.shippingAddress.addressLine2}</p>
                  )}
                  <p className="text-gray-600 dark:text-gray-400">
                  ...
                  {selectedOrder.items?.map((item, index) => (
                    <div key={index} className="flex items-center space-x-4 p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-gray-600 dark:text-gray-400">
                          Price: ${(item.product?.price || item.price || 0).toFixed(2)} each
                        </p>
                      </div>
                      <div className="text-right">
              ...
              <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                <button
                  onClick={() => showOrderTrackingInfo(selectedOrder)}
                  className="flex items-center bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 font-medium text-sm transition-colors"
                >
                  <Truck className="w-4 h-4 mr-2" />
                  Track Order
                ...
                <button
                  ...
                  className="flex items-center border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 font-medium text-sm transition-colors"
                >
                  <Download className="w-4 h-4 mr-2" />
                  Download Invoice
                </button>
