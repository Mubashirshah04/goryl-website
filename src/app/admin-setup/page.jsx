...
import { doc, updateDoc, getDoc, setDoc } from '@/lib/firestore';
      ...
      <div className="max-w-2xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="space-y-4">
            <button onClick={checkUserRole} disabled={loading} className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
              <User className="w-4 h-4"/>
              <span>{loading ? 'Checking...' : 'Check Current Role'}</span>
            </button>

            {userExists === false && (<button onClick={createUserDocument} disabled={loading} className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
                <CheckCircle className="w-4 h-4"/>
                <span>{loading ? 'Creating...' : 'Create User Document'}</span>
              </button>)}

            {userExists && userRole !== 'admin' && (<button onClick={setAdminRole} disabled={loading} className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50">
                <Shield className="w-4 h-4"/>
                <span>{loading ? 'Setting...' : 'Set Admin Role'}</span>
              </button>)}

            {userExists && userRole === 'admin' && (<button onClick={removeAdminRole} disabled={loading} className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50">
          ...
          {/* Status Messages */}
          {userRole === 'admin' && (<div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          ...
          {userRole === 'user' && (<div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center space-x-2">
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <a href="/admin/applications" className="flex items-center justify-center space-x-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                <Shield className="w-4 h-4"/>
                <span>Admin Applications</span>
              </a>
              <a href="/test-applications" className="flex items-center justify-center space-x-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                <User className="w-4 h-4"/>
                <span>My Applications</span>
              </a>
              <a href="/become-seller" className="flex items-center justify-center space-x-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                <CheckCircle className="w-4 h-4"/>
                <span>Become a Seller</span>
              </a>
              <a href="/" className="flex items-center justify-center space-x-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                <span>Home</span>
              </a>
            </div>
          </div>
