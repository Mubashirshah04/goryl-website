...
import { 
  Smartphone, Laptop, Tv, Headphones, Camera, Watch, Shirt, ShoppingBag, Gem, Diamond,
  Home, Sofa, Lamp, ChefHat, Heart, Sparkles, Scissors, Palette, Dumbbell, Bike, 
  Gamepad2, Trophy, Search, BookOpen, GraduationCap, Car, PawPrint, Wrench, Baby, 
  Users, Music, Gift, Zap, Star, ChevronRight, Tablet, Printer, Speaker, Keyboard, 
  Mouse, Gamepad, Monitor, Server, Wifi, Shield, Code, Database, Cloud, Globe, 
  Coffee, Wine, Beer, Cake, Apple, Eye, Brain, Pill, Building, Factory, Hammer, 
  Paintbrush, Calculator, Microscope, Map, ShoppingCart, Package, Truck, Droplets,
  Clock, Leaf, Newspaper, TrendingUp, User, Thermometer, Volume2, 
  Mountain, Snowflake, Flag, Target, Puzzle, Blocks, 
  Milk, Beef, Wheat, Store, Settings, Stethoscope, Wind, Bandage, Circle, PenTool
} from 'lucide-react';
...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        const mod: any = await import('@/lib/services/categoryService');
        const firebaseCategories = mod?.getCategories ? await mod.getCategories() : [];
        
        // Merge Firebase data with default structure
        const mergedCategories = defaultCategories.map(defaultCat => {
          const firebaseCat = firebaseCategories.find((fc: any) => fc.slug === defaultCat.id);
          return {
            ...defaultCat,
            count: firebaseCat ? `${firebaseCat.productCount}+` : '0',
            realProductCount: firebaseCat?.productCount || 0
          };
        });
        ...
        setCategories(defaultCategories.map(cat => ({ ...cat, count: '0', realProductCount: 0 })));
      } finally {
        setLoading(false);
      }
    };
              ...
              <div className="max-w-sm mx-auto">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-2 text-sm bg-white dark:bg-white/10 backdrop-blur-sm border border-gray-200 dark:border-white/20 rounded-lg focus:ring-2 focus:ring-purple-400/50 focus:outline-none text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-purple-200"
                  />
                </div>
              </div>
            </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[...Array(6)].map((_, index) => (
              <div key={index} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-3 h-full animate-pulse cursor-pointer hover:bg-white/10 hover:border-purple-400/30 transition-all duration-300">
                ...
                <div className="space-y-1">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="h-2 bg-gray-600 rounded"></div>
                  ))}
                </div>
              </div>
            ))}
          ...
          {filteredCategories.map((category: any, index: number) => (
            ...
            >
              <Link href={`/categories/${category.id}`}>
                ...
                {category.featured && (
                  <span className="inline-block bg-gradient-to-r from-purple-500 to-pink-500 text-white px-2 py-0.5 rounded-full text-xs font-medium mb-3 shadow">
                    ✨
                  </span>
                )}
                ...
                <div className="space-y-1 mb-3">
                  {category.subcategories.slice(0, 3).map((sub: any, subIndex: number) => (
                    <div key={subIndex} className="flex items-center justify-between text-xs bg-gray-50 dark:bg-white/5 rounded px-2 py-1">
