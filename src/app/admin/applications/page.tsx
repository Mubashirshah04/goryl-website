...
import { 
  Shield, Search, Filter, Download, Plus, Eye, CheckCircle, XCircle,
  Clock, AlertTriangle, FileText, Building, Crown, UserCheck, MapPin,
  Calendar, Mail, Phone, Globe, CreditCard, ExternalLink, ChevronDown,
  ChevronUp, FileImage, FileVideo, FileText as FilePdf, X, Send, MessageSquare,
  User, Users, Building2, Store, MoreHorizontal, Edit, Trash2, RefreshCw, Play
} from 'lucide-react'
...
interface Application {
  ...
  email: string
  phone: string
  type: 'personal' | 'brand' | 'company' | 'seller'
...
export default function ApplicationsPage() {
  ...
  const [isAdmin, setIsAdmin] = useState(true) // ✅ Set to true by default since AdminRouteGuard already verified
  const [checkingAuth, setCheckingAuth] = useState(false) // ✅ No need to check again

  // ✅ REMOVED: Admin check is already done by AdminRouteGuard component
  // If user reaches this page, they are already verified as admin
  ...
  useEffect(() => {
    ...
    const unsubscribe = getAllApplications((fetchedApplications) => {
      ...
      const transformedApplications: Application[] = fetchedApplications.map(app => {
        ...
        let mappedType: 'personal' | 'brand' | 'company' | 'seller' = app.type as any;
        ...
        if (!['personal', 'brand', 'company', 'seller', 'personal_seller'].includes(app.type)) {
          ...
          mappedType = app.type as 'personal' | 'brand' | 'company' | 'seller';
        }
        ...
        return {
          ...
          documents: Array.isArray(app.documents) ? app.documents.map(doc => ({
  ...
  const handleApplicationAction = async (applicationId: string, action: 'approved' | 'rejected' | 'incomplete' | 'resubmission', reason?: string) => {
    setActionLoading(applicationId)
    
    console.log('🔍 Handling application action:', { applicationId, action, reason });
    
    try {
      await updateApplicationStatus(applicationId, action, user?.sub || '', reason)
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'personal': return 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30'
      case 'brand': return 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30'
      case 'company': return 'bg-blue-500/20 text-blue-400 border-blue-500/30'
      case 'seller': return 'bg-purple-500/20 text-purple-400 border-purple-500/30'
  ...
  const updateDocumentVerificationLocal = async (applicationId: string, documentId: string, verified: boolean) => {
    try {
      // Call the imported function
      await updateDocumentVerification(applicationId, documentId, verified);
  ...
  return (
    <AdminLayout 
      title="Applications" 
      subtitle="Review seller, brand, and company applications"
      searchPlaceholder="Search applications by name, business, or category..."
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <motion.button
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white rounded-xl hover:from-purple-600 hover:to-fuchsia-600 transition-all duration-300 font-bold flex items-center space-x-2"
                      >
                  ...
                  <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {selectedApplication.documents.map((doc, index) => (
                        <div key={doc.id || `doc-${index}`} className="bg-white/5 rounded-2xl p-4 border border-white/10 hover:bg-white/10 transition-all duration-300">
                          {/* Document Header */}
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center space-x-3">
                              <div className={`p-2 rounded-xl ${
                                doc.verified ? 'bg-green-500/20' : 'bg-yellow-500/20'
                              }`}>
                                {getDocumentIcon(doc.type)}
                              </div>
                              <div>
                                <p className="text-white font-semibold text-sm">{doc.name}</p>
                                <p className="text-purple-200 text-xs">{formatDate(doc.uploadedAt)}</p>
                              </div>
                            </div>
                            {doc.verified ? (
                            ...
                            {doc.type === 'image' && (
                              <div className="relative group">
                                <div className="aspect-video bg-white/5 rounded-xl overflow-hidden border border-white/10">
                                  <img 
                                    src={doc.url || '/api/placeholder-image.jpg'} 
                                    alt={doc.name}
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                  <motion.button
                                    ...
                                    className="px-4 py-2 bg-white/20 backdrop-blur-sm text-white rounded-xl hover:bg-white/30 transition-all duration-300 font-semibold flex items-center space-x-2"
                                  >
                                    <Eye className="w-4 h-4" />
                                    <span>View Full</span>
                                  </motion.button>
                            ...
                            {doc.type === 'video' && (
                              <div className="bg-gradient-to-r from-blue-500/20 to-blue-600/20 rounded-xl p-4 border border-blue-500/30">
                                ...
                                <motion.button
                                  ...
                                  whileTap={{ scale: 0.98 }}
                                  onClick={() => window.open(doc.url, '_blank')}
                                  className="w-full mt-3 px-4 py-2 bg-blue-500/30 text-white rounded-xl hover:bg-blue-500/40 transition-all duration-300 font-semibold flex items-center justify-center space-x-2"
                                >
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                             ...
                             <motion.button
                               whileHover={{ scale: 1.05 }}
                               whileTap={{ scale: 0.95 }}
                               onClick={() => updateDocumentVerificationLocal(selectedApplication.id, doc.id, !doc.verified)}
                    ...
                    <div className="mt-6 pt-6 border-t border-white/10">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {['CNIC', 'NTN', 'Passport', 'Bank Statements'].map((category) => {
                          const categoryDocs = selectedApplication.documents.filter(doc => 
                            doc.name.toLowerCase().includes(category.toLowerCase())
                          );
                          const verifiedCount = categoryDocs.filter(doc => doc.verified).length;
                          
                          return (
                            <div key={category} className="bg-white/5 rounded-xl p-3 border border-white/10">
                  ...
                  {selectedApplication.notes.length > 0 && (
                    <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                      ...
                      <div className="space-y-2">
                        {selectedApplication.notes.map((note, noteIndex) => (
                          <div key={noteIndex} className="flex items-start space-x-3 p-3 bg-white/5 rounded-xl">
                    ...
                    <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                        <motion.button
                          ...
                          className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl hover:from-green-600 hover:to-emerald-600 transition-all duration-300 font-bold flex items-center space-x-2 disabled:opacity-50"
                        >
                        ...
                        <motion.button
                          ...
                          disabled={actionLoading === selectedApplication.id}
                          className="px-6 py-3 bg-red-500/20 text-red-400 rounded-xl hover:bg-red-500/30 transition-all duration-300 border border-red-500/30 font-bold flex items-center space-x-2 disabled:opacity-50"
                        >
                        ...
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleApplicationAction(selectedApplication.id, 'incomplete')}
                          disabled={actionLoading === selectedApplication.id}
                          className="px-6 py-3 bg-orange-500/20 text-orange-400 rounded-xl hover:bg-orange-500/30 transition-all duration-300 border border-orange-500/30 font-bold flex items-center space-x-2 disabled:opacity-50"
                        >
                        ...
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleApplicationAction(selectedApplication.id, 'resubmission')}
                          disabled={actionLoading === selectedApplication.id}
                          className="px-6 py-3 bg-blue-500/20 text-blue-400 rounded-xl hover:bg-blue-500/30 transition-all duration-300 border border-blue-500/30 font-bold flex items-center space-x-2 disabled:opacity-50"
                        >
