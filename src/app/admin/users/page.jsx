...
import { Users, Eye, UserX, Shield, Calendar, MapPin, Smartphone, Monitor, Tablet, ChevronDown, Download, Plus, User, UserCheck, Building, Crown, CheckCircle, XCircle, Clock, AlertTriangle, Ban } from 'lucide-react';
...
export default function UserManagementPage() {
    ...
    useEffect(() => {
        const loadUsers = async () => {
            setLoading(true);
            try {
                const { getDocs, collection } = await import('@/lib/firestore');
                ...
                const userList = querySnapshot.docs.map(doc => {
                    ...
                    return {
                        ...
                        avatar: data.avatar || '',
                        joinDate: data.createdAt ? new Date(data.createdAt.seconds * 1000) : new Date(),
                        lastLogin: data.lastLogin ? new Date(data.lastLogin.seconds * 1000) : new Date(),
    ...
    const handleUserAction = async (userId, action) => {
        ...
        setUsers(prev => prev.map(user => {
            if (user.id === userId) {
                return Object.assign(Object.assign({}, user), { status: action === 'activate' ? 'active' : action === 'suspend' ? 'suspended' : 'banned' });
            }
            return user;
        }));
    ...
    return (<AdminLayout title="User Management" subtitle="Manage platform users and permissions" searchPlaceholder="Search users by name, email, or location..." onSearch={handleSearch} actions={<div className="flex items-center space-x-3">
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={exportUsers} className="bg-white/10 text-white px-4 py-2 rounded-xl font-bold hover:bg-white/20 transition-all duration-300 border border-white/20 flex items-center space-x-2">
            <Download className="w-4 h-4"/>
            <span>Export</span>
          </motion.button>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white px-4 py-2 rounded-xl font-bold hover:from-purple-600 hover:to-fuchsia-600 transition-all duration-300 shadow-lg flex items-center space-x-2">
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="relative">
                <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-400 appearance-none pr-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {filteredUsers.map((user, index) => (<motion.tr key={user.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 + index * 0.1 }} className="border-b border-white/10 hover:bg-white/5 transition-colors cursor-pointer" onClick={() => {
                ...
                setSelectedUser(user);
                setShowProfileDrawer(true);
            }} className="p-2 bg-blue-500/20 rounded-xl hover:bg-blue-500/30 transition-colors border border-blue-500/30" title="View Profile">
                          <Eye className="w-4 h-4 text-blue-400"/>
                        </motion.button>
                        
                        {user.status === 'active' ? (<motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={(e) => {
                    e.stopPropagation();
                    handleUserAction(user.id, 'suspend');
                }} className="p-2 bg-yellow-500/20 rounded-xl hover:bg-yellow-500/30 transition-colors border border-yellow-500/30" title="Suspend User">
                            <UserX className="w-4 h-4 text-yellow-400"/>
                          </motion.button>) : (<motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={(e) => {
                    e.stopPropagation();
                    handleUserAction(user.id, 'activate');
                }} className="p-2 bg-green-500/20 rounded-xl hover:bg-green-500/30 transition-colors border border-green-500/30" title="Activate User">
            ...
            }} className="p-2 bg-red-500/20 rounded-xl hover:bg-red-500/30 transition-colors border border-red-500/30" title="Ban User">
        ...
        {/* Profile Drawer */}
        <AnimatePresence>
          {showProfileDrawer && selectedUser && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowProfileDrawer(false)}>
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                <div className="p-6 border-b border-white/20">
                  <div className="flex items-center justify-between">
                    ...
                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setShowProfileDrawer(false)} className="p-2 bg-white/10 rounded-xl hover:bg-white/20 transition-colors">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    ...
                    <div className="space-y-3">
                      {selectedUser.loginHistory.map((login) => (<div key={login.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                          <div className="flex items-center space-x-3">
                          ...
                          <div className="text-right">
                            ...
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${login.success
                  ...
                  <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {selectedUser.devices.map((device) => (<div key={device.id} className="flex items-center space-x-3 p-3 bg-white/5 rounded-xl">
                          <div className={`p-2 rounded-xl ${device.type === 'mobile' ? 'bg-blue-500/20' :
                    ...
                    device.type === 'desktop' ? <Monitor className="w-4 h-4 text-green-400"/> :
                        <Tablet className="w-4 h-4 text-purple-400"/>}
                          ...
                          <div className="text-right">
                            ...
                            {device.active && (<span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400 border border-green-500/30">
                                Active
                              </span>)}
                          </div>
                        </div>))}
