...
import { FileText, Eye, Download, Filter, ChevronDown, ChevronUp, User, Clock, CheckCircle, X } from 'lucide-react';
import AdminLayout from '@/components/layout/AdminLayout';
import { subscribeToAuditLogs } from '@/lib/adminService';
export default function AdminAuditPage() {
    ...
    const actions = (<div className="flex items-center space-x-2">
      <button onClick={exportAuditLogs} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
    ...
    return (<AdminLayout title="Audit Log" subtitle="Track all platform activities and changes" searchPlaceholder="Search audit logs by actor or action..." onSearch={handleSearch} actions={actions}>
        ...
        {showFilters && (<div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <select value={filters.action} onChange={(e) => handleFilterChange('action', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-black">
              ...
              <div>
                ...
                <select value={filters.targetType} onChange={(e) => handleFilterChange('targetType', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-black">
            ...
            if (e.target.checked) {
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (<tr>
                  ...
                  </tr>) : (auditLogs.map((log) => (<tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input type="checkbox" checked={selectedLogs.includes(log.id)} onChange={(e) => {
                if (e.target.checked) {
