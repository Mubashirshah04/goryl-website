...
import { Star, Eye, Trash2, Download, Filter, ChevronDown, ChevronUp, User, Package, Flag, AlertTriangle, CheckCircle } from 'lucide-react';
import { toast } from 'sonner';
import AdminLayout from '@/components/layout/AdminLayout';
import { subscribeToReviews, removeReview } from '@/lib/adminService';
export default function AdminReviewsPage() {
    ...
    const handleReviewAction = async (reviewId, action, details) => {
        try {
            if (action === 'remove') {
    ...
    const renderStars = (rating) => {
        ...
        {[1, 2, 3, 4, 5].map((star) => (<Star key={star} className={`w-4 h-4 ${star <= rating ? 'fill-current text-yellow-400' : 'text-gray-300'}`}/>))}
    ...
    const actions = (<div className="flex items-center space-x-2">
      <button onClick={exportReviews} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
    ...
    return (<AdminLayout title="Reviews" subtitle="Manage product reviews and moderation" searchPlaceholder="Search reviews by product or user..." onSearch={handleSearch} actions={actions}>
        ...
        {showFilters && (<div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <select value={filters.rating} onChange={(e) => handleFilterChange('rating', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-black">
            ...
            if (e.target.checked) {
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (<tr>
                  ...
                  </tr>) : (reviews.map((review) => (<tr key={review.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input type="checkbox" checked={selectedReviews.includes(review.id)} onChange={(e) => {
                if (e.target.checked) {
                    ...
                    setSelectedReviews(prev => prev.filter(id => id !== review.id));
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          {review.status === 'active' && (<button className="text-red-600 hover:text-red-900" onClick={() => handleReviewAction(review.id, 'remove', { reason: 'Admin removal' })}>
                              <Trash2 className="w-4 h-4"/>
                            </button>)}
                          {review.reportCount > 0 && (<button className="text-yellow-600 hover:text-yellow-900" onClick={() => { }}>
