...
import { 
  Shield, Search, Filter, Download, RefreshCw, Eye, AlertTriangle, 
  CheckCircle, XCircle, Clock, MapPin, Smartphone, Monitor, Tablet,
  User, Calendar, Globe, Lock, Unlock, Activity, Database, Zap
} from 'lucide-react'
...
const demoLoginAudits: LoginAudit[] = [];

export default function LoginAuditPage() {
  const [loginAudits, setLoginAudits] = useState<LoginAudit[]>(demoLoginAudits)
  const [filteredAudits, setFilteredAudits] = useState<LoginAudit[]>(demoLoginAudits)
  ...
  React.useEffect(() => {
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter(audit => 
        statusFilter === 'success' ? audit.success : !audit.success
      )
    }
  ...
  return (
    <AdminLayout 
      title="Login Audit" 
      subtitle="Monitor user login activity and security events"
      searchPlaceholder="Search by user, email, IP, or location..."
      ...
      actions={
        <div className="flex items-center space-x-3">
          <motion.button
            ...
            onClick={refreshData}
            disabled={loading}
            className="bg-white/10 text-white px-4 py-2 rounded-xl font-bold hover:bg-white/20 transition-all duration-300 border border-white/20 flex items-center space-x-2 disabled:opacity-50"
          >
                  ...
                  >
                    ...
                    <td className="p-6">
                      <div className="text-sm">
                        ...
                        {audit.logoutReason && (
                          <p className="text-purple-200 text-xs">{audit.logoutReason}</p>
                        )}
                      </div>
                    </td>
                  </motion.tr>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="space-y-3">
              {loginAudits.filter(audit => !audit.success).slice(0, 5).map((audit) => (
                <div key={audit.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                  <div>
          ...
          >
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={device.type} className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                  <div className="flex items-center space-x-3">
