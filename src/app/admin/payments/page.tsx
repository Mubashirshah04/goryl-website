...
import {
  CreditCard, Eye, CheckCircle, X, Download, Filter,
  ChevronDown, ChevronUp, DollarSign, Calendar, User, Clock,
  AlertCircle, TrendingUp, Building2
} from 'lucide-react'
...
export default function AdminPaymentsPage() {
  const [payments, setPayments] = useState<PaymentAnalytics[]>([])
  ...
  const [sellerSummaries, setSellerSummaries] = useState<SellerPaymentSummary[]>([])
  const [showHoldModal, setShowHoldModal] = useState(false)
  const [selectedSeller, setSelectedSeller] = useState<SellerPaymentSummary | null>(null)
  const [holdAmount, setHoldAmount] = useState('')
  const [holdReason, setHoldReason] = useState('')

  useEffect(() => {
    const unsubscribe = subscribeToPayments(filters, (newPayments) => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAllSellerPayments((summaries) => {
  ...
  const handlePaymentAction = async (paymentId: string, action: string, details?: any) => {
    try {
      if (action === 'approve' || action === 'reject' || action === 'pay') {
        await processWithdrawRequest(paymentId, action as any, 'admin', details)
      } else {
  ...
  return (
    <AdminLayout
      title="Payouts"
      subtitle="Manage seller payouts and payment processing"
      searchPlaceholder="Search payments by seller or ID..."
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-green-900">
                    {formatCurrency(payments.filter(p => p.status === 'requested').reduce((sum, p) => sum + p.amount, 0))}
                  </p>
                </div>
              </div>
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sellerSummaries.map((seller) => (
                  <tr key={seller.sellerId} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                {loading ? (
                  ...
                  payments.map((payment) => (
                    <tr key={payment.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(payment.requestedAt)}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            onClick={() => {/* Open payment detail drawer */}}
                          >
                            <Eye className="w-4 h-4" />
                          </button>
                          {payment.status === 'requested' && (
                            <>
                              ...
                              <button
                                className="text-red-600 hover:text-red-900"
                                onClick={() => handlePaymentAction(payment.id, 'rejected', { reason: 'Admin rejection' })}
                              >
                                <X className="w-4 h-4" />
                              </button>
                            </>
                          ...
                          {payment.status === 'approved' && (
                            <button
                              className="text-blue-600 hover:text-blue-900"
                              onClick={() => handlePaymentAction(payment.id, 'paid', { transactionId: `TXN${Date.now()}` })}
                            >
