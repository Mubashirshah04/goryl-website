...
export default function TestEmailPage() {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
        ...
        <button onClick={handleTestEmail} disabled={loading} className={`w-full py-3 px-4 rounded-xl font-semibold text-white transition-all duration-300 ${loading
        ...
        {result && (<div className={`mt-6 p-4 rounded-xl ${result.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
            ...
            <p className={`text-sm ${result.success ? 'text-green-700' : 'text-red-700'}`}>
              {result.success
                ...
                : `Error: ${((_a = result.error) === null || _a === void 0 ? void 0 : _a.message) || 'Unknown error'}`}
            </p>
          </div>)}
