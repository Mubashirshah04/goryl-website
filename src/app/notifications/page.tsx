...
interface Notification {
  id: string;
  type: 'order' | 'product' | 'social' | 'system' | 'payment' | 'shipping';
  title: string;
  message: string;
  timestamp: Date;
  read: boolean;
...
export default function NotificationsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'unread' | 'orders' | 'social' | 'system'>('all');
  ...
  const getNotificationType = (firebaseType: string): 'order' | 'product' | 'social' | 'system' | 'payment' | 'shipping' => {
    if (firebaseType.includes('order')) return 'order';
    if (firebaseType.includes('payment')) return 'payment';
    if (firebaseType.includes('ship')) return 'shipping';
    if (firebaseType.includes('product')) return 'product';
    if (firebaseType.includes('social')) return 'social';
    return 'system';
  };
  ...
  const getPriorityFromType = (type: string): 'low' | 'medium' | 'high' => {
    if (type.includes('cancelled') || type.includes('failed')) return 'high';
    if (type.includes('delivered') || type.includes('confirmed')) return 'medium';
    return 'low';
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            {unreadCount > 0 && (
              <motion.button
                ...
                className="bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white px-6 py-3 rounded-2xl hover:from-purple-600 hover:to-fuchsia-600 transition-all duration-300 shadow-2xl font-bold text-lg"
              >
                Mark All Read
              </motion.button>
            )}
        ...
        <motion.div
          ...
          className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6 mb-8 shadow-2xl"
        >
          <div className="flex flex-col lg:flex-row gap-4 items-center">
            ...
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                className="bg-white/10 border border-white/20 rounded-2xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent backdrop-blur-sm"
              >
            ...
            <AnimatePresence>
              {filteredNotifications.map((notification, index) => (
                <motion.div
                  ...
                  className={`bg-white/10 backdrop-blur-xl border rounded-3xl p-6 shadow-2xl hover:shadow-purple-500/20 transition-all duration-500 group ${
                    notification.read 
                      ? 'border-white/20' 
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        {!notification.read && (
                          <motion.button
                            ...
                            className="bg-white/10 text-purple-200 hover:text-white hover:bg-white/20 px-4 py-2 rounded-2xl transition-all duration-300 border border-white/20"
                          >
                            Mark Read
                          </motion.button>
                        )}
                        ...
                        <motion.button
                          ...
                          className="bg-red-500/20 text-red-400 hover:text-red-300 hover:bg-red-500/30 px-4 py-2 rounded-2xl transition-all duration-300 border border-red-500/30"
                        >
