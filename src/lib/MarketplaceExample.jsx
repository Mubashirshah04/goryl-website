...
import { marketplaceService, getCurrentUser, isSeller } from './index';
const MarketplaceExample = () => {
    ...
    const handleProductFormChange = (e) => {
        ...
        setProductForm(prev => (Object.assign(Object.assign({}, prev), { [name]: name === 'price' || name === 'stock' ? Number(value) : value })));
    };
    // Handle reel form changes
    const handleReelFormChange = (e) => {
        const { name, value } = e.target;
        setReelForm(prev => (Object.assign(Object.assign({}, prev), { [name]: value })));
    };
    ...
    const handleApplicationFormChange = (e) => {
        ...
        if (name.includes('.')) {
            ...
            setApplicationForm(prev => (Object.assign(Object.assign({}, prev), { [parent]: Object.assign(Object.assign({}, prev[parent]), { [child]: value }) })));
        }
        else {
            setApplicationForm(prev => (Object.assign(Object.assign({}, prev), { [name]: value })));
        }
    };
    // Create product
    const handleCreateProduct = async (e) => {
        e.preventDefault();
        try {
            const productId = await marketplaceService.createProduct(Object.assign(Object.assign({}, productForm), { images: [] // In a real app, you would upload images first
             }));
    ...
    const handleCreateOrder = async () => {
        try {
            ...
            const shippingAddress = {
                id: '',
                userId: (currentUser === null || currentUser === void 0 ? void 0 : currentuser.sub) || '',
                type: 'home',
                firstName: 'John',
                lastName: 'Doe',
    ...
    return (<div className="marketplace-example">
      ...
      <div className="user-info">
        ...
        {currentUser ? (<div>
          ...
          </div>) : (<p>Please sign in to use the marketplace</p>)}
      </div>
      
      {/* Seller Application (for non-sellers) */}
      {!isUserSeller && currentUser && (<div className="seller-application">
          <h2>Apply to Become a Seller</h2>
          <form onSubmit={handleSubmitApplication}>
            <div>
              <label>Account Type:</label>
              <select name="requestedType" value={applicationForm.requestedType} onChange={handleApplicationFormChange}>
            ...
            <div>
              ...
              <textarea name="businessDescription" value={applicationForm.businessDescription} onChange={handleApplicationFormChange} required/>
            </div>
            
            <div>
              <label>Phone:</label>
              <input type="text" name="phone" value={applicationForm.phone} onChange={handleApplicationFormChange} required/>
            </div>
            
            <h3>Address</h3>
            <div>
              <label>Street:</label>
              <input type="text" name="address.street" value={applicationForm.address.street} onChange={handleApplicationFormChange} required/>
            </div>
            
            <div>
              <label>City:</label>
              <input type="text" name="address.city" value={applicationForm.address.city} onChange={handleApplicationFormChange} required/>
            </div>
            
            <div>
              <label>State:</label>
              <input type="text" name="address.state" value={applicationForm.address.state} onChange={handleApplicationFormChange} required/>
            </div>
            
            <div>
              <label>ZIP Code:</label>
              <input type="text" name="address.zipCode" value={applicationForm.address.zipCode} onChange={handleApplicationFormChange} required/>
            </div>
            
            <div>
              <label>Country:</label>
              <input type="text" name="address.country" value={applicationForm.address.country} onChange={handleApplicationFormChange} required/>
            </div>
      ...
      {isUserSeller && (<div className="product-creation">
          ...
          <form onSubmit={handleCreateProduct}>
            ...
            <div>
              ...
              <input type="text" name="category" value={productForm.category} onChange={handleProductFormChange} required/>
            </div>
      ...
      {isUserSeller && (<div className="reel-creation">
          <h2>Create Reel</h2>
          <form onSubmit={handleCreateReel}>
            <div>
              <label>Video URL:</label>
              <input type="text" name="videoUrl" value={reelForm.videoUrl} onChange={handleReelFormChange} required/>
            </div>
            
            <div>
              <label>Caption:</label>
              <textarea name="caption" value={reelForm.caption} onChange={handleReelFormChange}/>
            </div>
      ...
      <div className="product-feed">
        ...
        <div className="products-grid">
          {products.map(product => (<div key={product.id} className="product-card">
              ...
              {currentUser && (<button onClick={() => handleAddToCart(product.id || '')}>
                  Add to Cart
                </button>)}
            </div>))}
        </div>
      ...
      {/* Cart */}
      {currentUser && cart && (<div className="cart">
      ...
      {/* Orders */}
      {currentUser && orders.length > 0 && (<div className="orders">
          <h2>My Orders</h2>
          <ul>
      ...
      <div className="reels">
        ...
        <div className="reels-grid">
          {reels.map(reel => (<div key={reel.id} className="reel-card">
              <video src={reel.videoUrl} controls/>
              <p>{reel.caption}</p>
              <p>Likes: {reel.likesCount}</p>
              {currentUser && (<button onClick={() => reel.id && handleLikeReel(reel.id)}>
                  Like
                </button>)}
            </div>))}
        </div>
      ...
      <div className="stories">
        ...
        <div className="stories-grid">
          {stories.map(story => (<div key={story.id} className="story-card">
              {story.mediaType === 'image' ? (<img src={story.storyImage} alt="Story"/>) : (<video src={story.storyVideo} controls/>)}
              <p>{story.userName}</p>
            </div>))}
        </div>
      </div>
