...
async function listCurrentIndexes(): Promise<IndexInfo[]> {
  try {
...
async function waitForIndexBuild(minutes: number = 10): Promise<boolean> {
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      const buildingIndexes = indexes.filter((index: IndexInfo) => 
        index.state === 'BUILDING' || index.state === 'CREATING'
      );
...
async function main() {
  ...
  readline.question('\nDo you want to proceed? (yes/no): ', async (answer: string) => {
    readline.close();
    ...
    try {
      ...
      if (indexes.length > 0) {
        ...
        indexes.forEach((index: IndexInfo, i: number) => {
